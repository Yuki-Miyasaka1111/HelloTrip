        ...
        <form id="createProjectForm" action="{{ route( "$route" ) }}" method="{{ $method }}" enctype="multipart/form-data">
            ...
            <div>
                ...
                <div class="form-group d-flex justify-start items-stretch">
                    <x-labels.label label="サムネイル画像" class="flex-wrap" alignItems="items-baseline"  />
                    <div class="d-flex flex-wrap">
                        <x-inputs.image name="thumbnail" multiple="False" />
                    </div>
                    @error('thumbnail')
                ...
                <div class="form-group d-flex justify-start">
                    ...
                    <div class="p-1">
                        <x-inputs.text name="name" width="420px" placeholder="施設名を入力(最大40文字)" />
                    </div>
                    @error('name')
