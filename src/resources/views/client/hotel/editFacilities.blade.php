        ...
        <div class="form-group d-flex justify-start ">
            ...
            <div class="d-flex flex-wrap">
                @foreach ($facilities as $facility)
                    <x-client.inputs.checkbox name="facilities[]" :value="$facility->id" :label="$facility->name" id="facility-{{ $facility->id }}" :checked="in_array($facility->id, old('facilities', $selected_facilities))" width="50%" />
                @endforeach
            </div>
        </div>
        ...
        <div class="form-group d-flex justify-start ">
            ...
            <div class="d-flex flex-wrap">
                @foreach ($amenities as $amenity)
                    <x-client.inputs.checkbox name="amenities[]" :value="$amenity->id" :label="$amenity->name" id="amenity-{{ $amenity->id }}" :checked="in_array($amenity->id, old('amenities', $selected_amenities))" width="50%" />
                @endforeach
            </div>
        </div>
