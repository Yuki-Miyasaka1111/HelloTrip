...
<x-partials.project-information-box title="特徴">
    <form action="{{ isset($hotel) ? route('project.hotel.updateFeatures', $hotel->id) : route('project.hotel.storeFeatures') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group d-flex justify-start items-center ">
            ...
            <x-inputs.select name="prefecture_id" selectedOption="{{ $hotel->prefecture_id }}" width="fit-content" placeholder="地域を選択してください">
                @foreach ($prefectures as $prefecture)
                    <option value="{{ $prefecture->prefecture_id }}" @if($prefecture->prefecture_id == $hotel->prefecture_id) selected @endif>{{ $prefecture->prefecture_name }}</option>
                @endforeach
            </x-inputs.select>
            @error('prefecture_id')
