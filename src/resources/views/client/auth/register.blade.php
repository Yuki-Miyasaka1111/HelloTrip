<x-guest-layout>
    <form method="POST" action="{{ route('client.register') }}">
        ...
        <!-- 会社名 -->
        <div>
            <x-client.labels.input for="name" :value="__('会社名')" />
            ...
            <x-client.labels.input :messages="$errors->get('name')" class="mt-2" />
        </div>

        <!-- ご担当者名 -->
        <div class="mt-2">
            <x-client.labels.input for="manager_name" :value="__('ご担当者名')" />
            <x-client.inputs.text 
                id="manager_name" 
                class="block mt-1 w-full" 
                type="text" 
                ...
                required autofocus autocomplete="name" />
            <x-client.labels.input :messages="$errors->get('manager_name')" class="mt-2" />
        </div>

        <!-- ご担当者所属部署 -->
        <div class="mt-2">
            <x-client.labels.input for="manager_department" :value="__('ご担当者所属部署')" />
            <x-client.inputs.text 
                ...
                required autofocus autocomplete="name" />
            <x-client.labels.input :messages="$errors->get('manager_department')" class="mt-2" />
        </div>

        <!-- ご担当者連絡先 -->
        <div class="mt-2">
            <x-client.labels.input for="manager_phone_number" :value="__('ご担当者連絡先')" />
            <x-client.inputs.text 
                ...
                required autofocus autocomplete="name" />
            <x-client.labels.input :messages="$errors->get('manager_phone_number')" class="mt-2" />
        </div>
        ...
        <!-- Confirm Password -->
        <div class="mt-2">
            <x-client.labels.input for="password_confirmation" :value="__('確認用パスワード')" />

            <x-client.inputs.text id="password_confirmation" class="d-block mt-1 width-full"
                type="password"
                name="password_confirmation" 
                placeholder="半角英数字8文字以上" 
                required autocomplete="new-password" />

            <x-client.labels.input :messages="$errors->get('password_confirmation')" class="mt-1" />
        </div>
